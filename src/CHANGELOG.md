...
1. **Settings - Languages page** - Added dynamic language management under `/settings/languages`; loads `tenant.languages[]` from `GET /api/tenant` and all system languages from `GET /api/tenant/languages/available`; each configured language shows an active toggle (disabled for the default), a "Default" radio (auto-activates the language when set), and a remove button (disabled for the default or when only one language remains); available languages not yet added are listed as "+ Add" options; saving calls `PUT /api/tenant/languages` with the full array; client-side validation enforces at least one language, exactly one default, and the default must be active.
2. **Types - TenantLanguage / AvailableLanguage** - Added `TenantLanguage` and `AvailableLanguage` interfaces; extended `Tenant` type with `languages: TenantLanguage[]`; added `"sq"` (Albanian) to the `Language` union.
3. **Services - Language API** - Added `getAvailableLanguages()` (`GET /tenant/languages/available`) and `updateTenantLanguages()` (`PUT /tenant/languages`) to `authService.ts`.

4. **Invoices - Detail page** - Added `InvoiceDetailPage` (`/invoices/:id`) as a read-only detail view showing all invoice fields, line items table with totals, customer snapshot, payment info, notes, and linked jobs count.
5. **Invoices - View button** - Each invoice row now has a View button that navigates to `/invoices/:id` passing the invoice via router state for instant display (background fetch refreshes from the API).
6. **Invoices - Edit from detail** - The Edit button on the detail page navigates back to `/invoices` with the invoice in router state (`{ editInvoice }`) so the full `InvoiceFormSection` (with send, PDF download, line items, etc.) opens automatically.
7. **Invoices - Menu navigation fix** - Clicking the Invoices menu item from the detail page or while the edit form is open now correctly returns to the invoice table (effect re-runs on `location.key` change and resets form state when no edit intent is present in the navigation state).
