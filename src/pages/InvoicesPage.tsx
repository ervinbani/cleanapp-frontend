  ...
  useEffect(() => {
    apiClient
      ...
      .then((res) => {
        ...
        ((res.data as { data: DropdownCustomer[] }).data ?? []).forEach((c) => {
          if (c.phone) map[c._id] = c.phone;
        });
        setCustMap(map);
      })
      .catch(() => {});
        ...
        <>
          ...
          <div className={styles.toolbar}>
            ...
            >
              ...
              {STATUS_ORDER.map((s) => (
                <option key={s} value={s}>
                  {l[`status_${s}` as keyof typeof l] as string}
                </option>
              ))}
            </select>
            {/* Date filters */}
                            ...
                            {openMenuId === inv._id && (
                              <div className={styles.rowMenuDropdown}>
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    setOpenMenuId(null);
                                    navigate(`/invoices/${inv._id}`, { state: { invoice: inv } });
                                  }}
                                >
                                ...
                                {/* WhatsApp */}
                                {inv.status !== "void" && (() => {
                                  const custObj = getCustomer(inv);
                                  const custId = typeof inv.customerId === "string" ? inv.customerId : custObj?._id;
                                  const phone = custObj?.phone ?? (custId ? custMap[custId] : undefined);
                                  const msg = buildWhatsAppMessage(inv, lang);
                                  return (
                                    <button
                                      className={`${styles.rowMenuItem} ${styles.rowMenuItemWhatsApp}`}
                                      onClick={() => {
                                        setOpenMenuId(null);
                                        if (!phone) { alert(l.sendNoPhone); return; }
