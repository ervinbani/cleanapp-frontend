...
import { uploadFile, getReadUrl, deleteFile, listFiles } from "../services/uploadService";
...
export default function DocumentsPage() {
  ...
  const [activeCursorResource, setActiveCursorResource] = useState<UploadResource | null>(null);
  ...
  const fetchDocs = useCallback(async (nextCursor?: string, cursorResource?: UploadResource) => {
    if (nextCursor && cursorResource) {
      // load-more for a specific resource
      setLoadingMore(true);
      try {
        const result = await listFiles({ resource: cursorResource, cursor: nextCursor });
    ...
    try {
      const results = await Promise.all(
        RESOURCES.map((r) => listFiles({ resource: r }).catch(() => ({ files: [], count: 0, hasMore: false, nextCursor: null })))
      );
      ...
      setActiveCursorResource(firstWithMore ? RESOURCES[results.indexOf(firstWithMore)] : null);
    } catch {
      setListError(t("Failed to load documents.", "Error al cargar los documentos."));
    } finally {
      setLoading(false);
    }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.size > MAX_MB * 1024 * 1024) {
      setError(t(`File exceeds ${MAX_MB} MB limit.`, `El archivo supera el límite de ${MAX_MB} MB.`));
      setSelectedFile(null);
      return;
    }
    setError("");
  ...
  const handleUpload = async () => {
    if (!selectedFile || !refId.trim()) {
      setError(t("Please select a file and enter a reference ID.", "Selecciona un archivo e ingresa un ID de referencia."));
      return;
    }
    setError("");
    setUploading(true);
    setProgress(0);
    try {
      const key = await uploadFile(resource, refId.trim(), selectedFile, setProgress);
      ...
      setError(t("Upload failed. Please try again.", "Error al cargar. Inténtalo de nuevo."));
    } finally {
      setUploading(false);
      setProgress(0);
    }
  ...
  return (
    <div className={styles.page}>
      ...
      {/* Upload card */}
      {showForm && <div className={styles.card}>
        <h2 className={styles.cardTitle}>{t("Upload a file", "Cargar archivo")}</h2>
        ...
        <div className={styles.formRow}>
          ...
          <div className={styles.formGroup}>
            <label className={styles.label}>{t("Reference ID", "ID de referencia")}</label>
            <input
        ...
        >
          ...
          {selectedFile
            ? <span className={styles.dropzoneFile}>{selectedFile.name} ({(selectedFile.size / 1024).toFixed(1)} KB)</span>
            : <span>{t("Click or drag a file here", "Haz clic o arrastra un archivo aquí")}</span>
          }
          <input
            ref={fileInputRef}
            type="file"
        ...
        <div className={styles.actions}>
          <button className={styles.cancelBtn} onClick={handleCancel} disabled={uploading}>
            {t("Cancel", "Cancelar")}
          </button>
          <button
          ...
          >
            {uploading ? `${t("Uploading", "Cargando")}… ${progress}%` : t("Upload", "Cargar")}
          </button>
        </div>
      </div>}
      ...
      {loading ? (
        ...
        <div className={styles.card}>
          <h2 className={styles.cardTitle}>{t("Uploaded files", "Archivos cargados")}</h2>
          ...
          {hasMore && (
            <div className={styles.loadMoreRow}>
              <button
                ...
                onClick={() => cursor && activeCursorResource && fetchDocs(cursor, activeCursorResource)}
                disabled={loadingMore}
              >
                {loadingMore ? t("Loading…", "Cargando…") : t("Load more", "Cargar más")}
              </button>
            </div>
          )}
        </div>
      ) : (
        <p className={styles.empty}>{t("No documents uploaded yet.", "Aún no hay documentos cargados.")}</p>
      )}
    </div>
  );
}
